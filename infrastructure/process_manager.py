...
class ProcessManager:
    ...
    def _release_permits(self, count: int) -> None:
        """Release up to ``count`` semaphore permits."""
    ...
    async def run_command(
        ...
        **kwargs,
        ...
        try:
            ...
            # On task cancellation, ensure child process does not outlive the caller.
            logging.debug(f"Command cancelled, terminating process: {' '.join(cmd)}")
            try:
            ...
            try:
                ...
                logging.error(f"Failed to reap cancelled process: {process.pid}")
            raise
        finally:
    ...
    async def cleanup(self) -> None:
        ...
        async with self._lock:
            ...
            # Since we cleared tracking preemptively, unregister() will not release later.
    ...
    def cleanup_sync(self) -> None:
        ...
        if processes:
            self._release_permits(len(processes))
